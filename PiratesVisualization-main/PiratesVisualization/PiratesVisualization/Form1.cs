...
namespace PiratesVisualization
{
    public partial class Form1 : Form
    {
        ...
        private void OprimizePaths()
        {
            if (solver != null && solver.bestPaths != null && solver.bestPaths.Length > 0)
            {
                Debug.WriteLine("Applying 2-Opt optimization to best paths...");
                for (int i = 0; i < solver.bestPaths.Length; i++)
                {
                    Path currentBestPath = solver.bestPaths[i];
                    if (currentBestPath != null && currentBestPath.islandsPath != null && currentBestPath.islandsPath.Count > 0)
                    {
                        List<int> pathNodes = new List<int>();
                        pathNodes.Add(currentBestPath.islandsPath[0].from);
                        foreach (var segment in currentBestPath.islandsPath)
                        {
                            pathNodes.Add(segment.to);
                        }

                        int maxPasses = 1000;
                        PathOptimizer.OptimizePath(pathNodes, islandsPoints, maxPasses);

                        // Reconstruct the Path object from the optimized node list
                        List<(int from, int to)> optimizedSegments = new List<(int from, int to)>();
                        for (int k = 0; k < pathNodes.Count - 1; k++)
                        {
                            optimizedSegments.Add((pathNodes[k], pathNodes[k + 1]));
                        }

                        currentBestPath.UpdateFromSegments(optimizedSegments, solver.edges, solver.islands);
                    }
                }
            }
        }
                ...
                using (Font font = new Font("Arial", gradeFontSize))
                {
                    ...
                    for (int i = 0; i < numIslands; i++)
                    {
                        ...
                        if (startingIslandColors.TryGetValue(i, out Color startingColor))
                        {
                            ...
                            islandSize = (int)(15 * scale); // Size of the start island circle radius
                        }
                        else
                        {
                            // If it's not a starting island, use the default blue brush
                            currentIslandBrush = islandBrush;
                            islandSize = (int)(10 * scale); // Size of the island circle radius
                        }
