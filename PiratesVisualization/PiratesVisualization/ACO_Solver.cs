...
namespace PiratesVisualization
{
    internal class ACO_Solver
    {
        ...
        private const double BETA = 0.05;  // Influence of heuristic
        ...
        private int ChoseNextIsland(int island, List<int> allowed)
        {
            ...
            foreach (int nextIsland in allowed)
            {
                ...
                // add expected value modefier if more than 0
                if (islands[nextIsland].ExpectedVal() > 0)
                    heuristic += islands[nextIsland].ExpectedVal(); // adition to make sure expected values below 1
                                                                    // wont damage the result
